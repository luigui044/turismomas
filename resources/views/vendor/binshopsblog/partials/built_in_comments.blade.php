    ...
    <div class="card bg-light mb-3">
        <div class="card-header">
            ...
            @if (config('binshopsblog.comments.ask_for_author_website') && $comment->author_website)
                (<a href='{{ $comment->author_website }}' target='_blank' rel='noopener'>Sitio web</a>)
            @endif

            <span class="float-right"
                title='{{ $comment->created_at }}'><small>{{ $comment->created_at->diffForHumans() }}</small></span>
        </div>
...
@if (count($comments) > config('binshopsblog.comments.max_num_of_comments_to_show', 500) - 1)
    <p><em>Solo los primeros {{ config('binshopsblog.comments.max_num_of_comments_to_show', 500) }} comentarios
        se muestran</em>
    </p>
@endif
